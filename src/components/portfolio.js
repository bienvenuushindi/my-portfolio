                ...
                {projects.map(item => <div key={item.name} className="bg-secondary flex items-center shadow-lg border border-primary relative project-container">
                   <img src={item.screenshot} alt={item.name} className="h-72 w-full"/>
                    <div className="absolute top-0 w-full text-center text-white h-full bg-secondary flex flex-col items-center justify-around px-1 project-element">
                        ...
                        <ul className="flex flex-wrap gap-1 text-primary items-center justify-center">
                            {item.techStack.map(item => <li key={item} style={{padding: '2px', fontSize: '12px'}} className="rounded bg-black shadow uppercase scale-90 font-bold">{item}</li>)}
                        </ul>
                        <ul className="flex flex-wrap gap-1 text-secondary">
                            <li><a href={item.live} target="_blank" rel="noreferrer noopener" className="py-1 px-3 font-bold rounded bg-primary">Live</a></li>
                            <li><a href={item.github} target="_blank" rel="noreferrer noopener" className="py-1 px-3 font-bold rounded bg-primary">Github</a></li>
                        </ul>
                    </div>
                    <div className="absolute bottom-2 right-1 p-2 plus-icon shadow-secondary shadow-lg rounded-full bg-secondary text-primary text-2xl font-bold"><FaHandPointer className="bip" /></div>
                </div>
                )}
            </div>
        </section>
